...
const submitDoc = async () => {
    ...
    fileInput.type = 'file';
    fileInput.accept = '.pdf,.txt'; // 只接受 PDF 和 TXT 文件
...
const queryDatabase = async () => {
    ...
    dbQueryResponse.value = ''

    try {
        const response = await axios.get('/debug/points');
        dbQueryResponse.value=response.data;
        console.log(response)
...
const fetchHistory = async () => {
    ...
    try {
        ...
        // 假设返回的数据结构是 { history: "历史记录内容" }
        const historyData = response.data.history || response.data;
        
        // 显示在响应区域
        llmtext.value = historyData;
        
        toast.add({
            severity: 'success', 
            summary: 'Success', 
            detail: 'History fetched successfully', 
            life: 3000
        });
    } catch (err) {
...
<template>
  <div class="max-w-6xl mx-auto my-5">
    <Toast ref="toast" />
        ...
        <header class="gradient-bg text-white rounded-t-xl p-4 flex items-center justify-between">
            <div class="flex items-center">
                ...
                <h1 class="text-3xl font-bold"> RAG  </h1>
            </div>
                 ...
                 <form @submit.prevent="getLLM">
                    <div class="mb-6">
                        <label class="block text-indigo-700 font-semibold mb-2" for="message">
                            <i class="fas fa-comment mr-2"></i>输入信息:
                        </label>
                        <div class="flex">
                            <Textarea 
                            ...
                            <Button  @click="fetchHistory"  class="get-history-btn bg-sky-300! text-white py-4 px-6 rounded-r-xl text-lg font-semibold"  type="button"  :disabled="loading"  > {{ loading ? 'Processing...' : 'History' }} </Button>
                        </div>
                    </div>
                </form>
                    ...
                    <div class="mb-12">
                        <label class="block text-indigo-700 font-semibold mb-2" for="response">
                            <i class="fas fa-reply mr-2"></i>AI 响应:
                        </label>
                        <Textarea 
                            id="response"
                            v-model="llmtext"
                            rows="18" 
                            readonly class="response-area w-full p-5 border border-indigo-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none text-lg"
                        </Textarea>
                    </div>
                    ...
                    <div class="mb-6">
                        <label class="block text-indigo-700 font-semibold mb-2" for="url">
                            <i class="fas fa-link mr-2"></i>输入 URL ，加载到数据库
                        </label>
                        <div class="flex flex-col">
                            ...
                            <div class="flex">
                                <button 
                                    @click="submitUrls"
                                    :loading="loadingUrls"
                                    class="url-btn flex-1 text-white bg-fuchsia-400 py-3 px-6 rounded-bl-xl font-semibold"
                                >
                                ...
                                <button 
                                    @click="submitDoc"
                                    :loading="loadingUrls"
                                    class="doc-btn flex-1 py-3 bg-sky-400 text-white rounded-br-xl font-semibold"

                                >
                                    {{ loadingUrls ? 'Injecting...' : 'Submit Doc' }} 
                                </button>
                            </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex">
                            ...
                            <button @click="getMetadataCounts" :disabled="loadingMetadata" class="db-btn flex-1 py-3 bg-purple-400 text-white rounded-br-xl font-semibold">
                                <i class="fas fa-info-circle mr-2"></i>{{ loadingMetadata ? 'Loading...' : '获取元数据' }}
                            </button>
                        </div>
                    </div>
                    <!-- Metadata Dash -->
    ...
    <style>
        ...
        .submit-btn, .url-btn, .doc-btn,.db-btn {
            transition: all 0.3s ease;
        }
        .submit-btn:hover, .url-btn:hover, .doc-btn:hover,.db-btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 4px 6px -1px rgba(222, 61, 216, 0.1), 0 2px 4px -1px rgba(197, 88, 177, 0.06);
        }
        .get-history-btn, .url-btn, .db-btn {
            transition: all 0.3s ease;
        }
        .get-history-btn:hover, .url-btn:hover, .doc-btn:hover,.db-btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 4px 6px -1px rgba(59, 130, 246, 0.1), 0 2px 4px -1px rgba(37, 99, 235, 0.06);
        }
